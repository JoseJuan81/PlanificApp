<template>
  <nav data-cy="treeListMovilMenu" class="lateral-nav">
    <ul>
      <li
        v-for="(route, indexR) in routes"
        :key="indexR"
        :class="{ 'selected': $route.fullPath.includes(route.path) }"
        @click="goTo(route)"
      >
...
function data() {
  return {
    routes: [
      { path: '/actividades', title: 'Jerarquía' },
      { path: '/iconos', title: 'Íconos' },
